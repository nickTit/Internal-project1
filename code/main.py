...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        initial_count = await database.fetch_val(
        ...
        if initial_count is None:
            ...
            print(
                "Таблица 'visits' инициализирована начальной строкой (id=1, count=0)."
            )
        else:
            print(
                f"Таблица 'visits' уже содержит данные. Текущий счетчик: {initial_count}"
            )
...
app = FastAPI(
    title="PostgreSQL Visitor Counter API", version="1.0.0", lifespan=lifespan
)
...
async def increment_visitor_count():
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail={
                "status": "error",
                "db_error": "Не удалось выполнить операцию с DB.",
            },
        )
